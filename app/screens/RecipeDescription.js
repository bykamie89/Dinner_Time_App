...
export default class RecipeDescription extends Component {
  ...
  async getInstructions() {
    ...
    try {
      const reqInfo = await fetch(
        `${settings.URL}${id}/information?apiKey=${settings.API_KEY}`
      );
  ...
  renderInstruction({ item }) {
    return <InstructionsComponent number={item.number} step={item.step} />;
  }
    ...
    return (
      <SafeAreaView style={{ flex: 1 }}>
        <View style={styles.header}>
          <Image
            style={styles.img}
            source={{ uri: `https://spoonacular.com/recipeImages/${img}` }}
          />
        </View>
        <ScrollView style={styles.containerInfo}>
          ...
          <View style={{ marginHorizontal: 50 }}>
            ...
            <View
              style={[
                styles.horizontal,
                { marginTop: 16, justifyContent: 'space-around' },
              ]}
            >
